...
Run this after any formatting/generation pass. It is idempotent, so running it
repeatedly is safe. It fixes the things that break the published site
(https://ijk37.com/reinforcement-learning/):
  ...
  2. Adds the `markdown` attribute to centered nav divs so the links inside
     get processed.
  ...
  4. Fixes note image refs to assets/images/ (bare *.png -> ../assets/images/*.png).
  5. Ensures the "View the Live Site" badge sits under the banner in every
     README (re-inserts it if a formatting pass strips it).

The 04-resources page lives at 04-resources/README.md (published on the site and
...
LIVE_SITE_BADGE = (
    ...
    '(https://img.shields.io/badge/%F0%9F%A4%96_View_the_Live_Site-IJK37.COM-22D3EE'
    '?style=for-the-badge&labelColor=5B4BD6)]'
    '(https://ijk37.com/reinforcement-learning/)'
)


def ensure_live_site_badge(s: str) -> str:
    """Insert the neon live-site badge on the line after the banner, once."""
...
def fix_file(path: str) -> bool:
    ...
    orig = s
    depth = path.replace("\\", "/").count("/")   # 01-notes/x.md -> 1 ; 03-projects/n/README.md -> 2

    s = convert_nav(s)

    # neon live-site badge under the banner (README pages only)
    if path.replace("\\", "/").endswith("README.md"):
        s = ensure_live_site_badge(s)

    # quiz README -> quiz hub (the quiz app's index.html owns that folder)
